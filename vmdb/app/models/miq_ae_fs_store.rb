module MiqAeFsStore
  ...
  module ClassMethods
    ...
    def git_repository(domain, create = false)
      ...
      if domain_dir_name
        domain_dir = File.join(BASE_DIR, "#{domain_dir_name}")
        git_repo = MiqAeGit.new(:path => domain_dir)
      else
        return nil unless create
        domain_dir = File.join(BASE_DIR, "#{domain}")
    ...
    def domain_dir(domain)
      dir_name = "#{domain}"
      Dir.entries(BASE_DIR).detect { |d| d.casecmp(dir_name) == 0 }
    end
    ...
    def delete_directory(full_name)
      ...
      if dir.empty?
        ...
        git_repo.commit("Removing Directory #{dir}")
      end
    end
    ...
    def rename_ae_dir(domain, old_dir, new_dir, entry = nil)
      ...
      git_repo.commit("Moved directory #{old_dir} to #{new_dir}")
    end
    ...
    def delete_file(full_name)
      ...
      git_repo.commit("Removing File #{fname}")
    end
    ...
    def add_files_to_repo(domain, entries)
      ...
      git_repo.commit("Files Added #{entries.collect { |e| e[:path] }}")
    end
    ...
    def move_files_in_repo(domain, entries)
      ...
      git_repo.commit("Files Moved #{entries.collect { |e| e[:old_path] }.join('\n')}")
    end
    ...
    def load_ae_entry(git_repo, entry, klass = nil)
      ...
      # attrs.merge!(git_repo.file_attributes(entry[:rel_path]))
