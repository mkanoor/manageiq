...
class MiqAeMethod < MiqAeBase
  ...
  def write_original(hash, sub_dir)
    entry = []
    entry[0] = {:path => self.class.relative_filename(sub_dir, "#{name}.yaml"),
                :data => hash.to_yaml}
    entry[1] = new_method_entry(entry[0]) if location == "inline" && data.present?
  ...
  def write_renamed(hash, sub_dir)
    ...
    entry[0] = {:path     => self.class.relative_filename(sub_dir, "#{name}.yaml"),
                ...
                :data     => hash.to_yaml}
    entry[1] = rename_method_entry(entry[0]) if location == 'inline'
