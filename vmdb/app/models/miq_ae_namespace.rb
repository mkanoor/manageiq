class MiqAeNamespace < MiqAeBase
  ...
  def mv_domain_dir(old_name, entry)
    ...
    generate_id
    self.class.add_files_to_repo(domain_value, [entry])
    old_dir = Dir.entries(self.class.base_dir).detect { |d| d.casecmp(old_name) == 0 }
    FileUtils.mv(File.join(self.class.base_dir, old_dir), File.join(self.class.base_dir, name)) == 0 if old_dir
  end
  ...
  def self.fqname_to_filename(fqname)
    ...
    domain_or_namespace_file(git_repo, path)
  end
